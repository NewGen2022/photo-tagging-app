...
const Header = () => {
    ...
    return (
        <div
            className={`flex items-center place-content-between px-16 min-h-16 ${
                theme === 'dark' ? 'bg-slate-700' : 'bg-slate-200'
            }`}
        >
            <a href="/" id="back-logo" className="flex items-center">
                <div
                    className={`text-2xl font-bold group hover:text-cyan-400 ${
                        theme === 'dark' ? '' : 'text-slate-700'
                    } `}
                >
                    Pixel
                    <span
                        className={`text-cyan-400 group-hover:text-slate-100 ${
